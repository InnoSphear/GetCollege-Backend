...
app.get("/api/npf-proxy", async (req, res) => {
  try {
    const widgetId =
      req.query.w || "ae2953a41c5d2fd23c3d93c3b1b8185d";

    const response = await axios.get(
      `https://widgets.in4.nopaperforms.com/js/emwgts.js?w=${widgetId}`,
      {
        headers: {
          Accept: "*/*",
        },
    ...
    res
      .status(500)
      .send("console.error('NPF Widget Error');");
  }
});
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      console.log(
        `Server started on ${PORT} with database connection`
      );
    });
  } catch (error) {
    console.error(
      "Server startup aborted because database connection failed."
    );
    process.exit(1);
  }
};
